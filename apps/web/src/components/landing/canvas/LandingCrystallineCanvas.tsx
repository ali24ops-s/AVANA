      ...
      if (isVisible && isTabActive && !reducedMotion && ctx) {
        ...
        for (let ring = 1; ring <= ringCount; ring++) {
          ...
          for (let i = 0; i < nodeCount; i++) {
            const angle = (i / nodeCount) * Math.PI * 2 + (ring % 2 === 0 ? time * 0.04 : -time * 0.04);
        ...
        // Draw Structured Connection Lines
        for (let i = 0; i < nodes.length; i++) {
          for (let j = i + 1; j < nodes.length; j++) {
            ...
            if (dist < connectThreshold) {
              const alpha = (1 - dist / connectThreshold) * 0.22;
              ctx.beginPath();
  ...
  return (
    ...
    >
      {canvasSupported && !reducedMotion && (
        <canvas
          ref={canvasRef}
          className="absolute inset-0 w-full h-full block opacity-65"
        />
      )}

      {(!canvasSupported || reducedMotion) && (
        <div className="absolute inset-0 w-full h-full flex items-center justify-center opacity-50">

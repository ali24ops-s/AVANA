...
import { useTagWithPosts } from "../../hooks/useBlog.js";
...
export function BlogTagPage() {
  ...
  const { data, isLoading, isError } = useTagWithPosts(slug, {
    page,
    pageSize,
  });
    ...
    <div className="min-h-screen bg-[var(--color-bg-default)] text-[var(--color-text)] font-sans selection:bg-[#008080]/20 selection:text-[#008080]" dir="rtl">
      ...
      {tag && (
        <BlogSEO
          ...
          description={`مجموعه مقالات آموزشی و تخصصی داروسازی با برچسب ${tag.name} در پلتفرم آوانا.`}
          type="website"
          keywords={[tag.name]}
        />
      )}
        ...
        <header className="rounded-[20px] bg-[var(--color-surface)] border border-[var(--color-border)] p-8 sm:p-12 shadow-xs relative overflow-hidden">
          ...
          <div className="relative space-y-4 max-w-3xl">
            ...
            <p className="text-sm sm:text-base text-[var(--color-text-muted)] leading-relaxed font-normal">
              آرشیو تمام مقالات علمی و آموزشی داروسازی مرتبط با برچسب #{tag?.name || ""}.
            </p>

            <div className="pt-2 text-xs text-[#008080] font-semibold">
              {toPersianDigits(totalCount)} مقاله منتشر شده با این برچسب
            </div>
          </div>
        </header>
        ...
        <section className="space-y-6">
          {isLoading ? (
            ...
            <div className="text-center py-16 rounded-[16px] bg-[var(--color-surface)] border border-red-200 p-8 max-w-lg mx-auto space-y-4 shadow-xs">
              ...
              <h3 className="text-lg font-bold text-[var(--color-text)]">برچسب یافت نشد</h3>
              <p className="text-xs text-[var(--color-text-muted)]">
                این برچسب وجود ندارد یا هنوز مقاله‌ای با این برچسب منتشر نشده است.
              </p>
              <Link
                to="/blog"
            ...
            <div className="text-center py-20 rounded-[16px] bg-[var(--color-surface)] border border-[var(--color-border)] p-8 max-w-lg mx-auto space-y-3 shadow-xs">
              ...
              <h3 className="text-lg font-bold text-[var(--color-text)]">هنوز مقاله‌ای با این برچسب منتشر نشده است</h3>
              <p className="text-xs text-[var(--color-text-muted)]">
                به‌زودی مقالات جدیدی با برچسب #{tag.name} اضافه خواهد شد.
              </p>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-8">
              <button
                ...
                disabled={page === 1}
                className="p-2 rounded-[8px] bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-text)] disabled:opacity-30 transition-colors shadow-xs cursor-pointer"
                aria-label="صفحه قبل"
              >

...
export function AdminUserWalletTopupsPage() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "خطا در دریافت لیست شارژهای کیف پول";
      setErrorMsg(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleApprove = async (paymentId: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "خطا در تأیید شارژ کیف پول";
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-[var(--color-text)] flex items-center gap-2.5">
          <Wallet className="w-7 h-7 text-[var(--color-primary-default)]" />
          شارژ کیف پول
        </h1>
        <p className="text-sm text-[var(--color-text-muted)]">
          مشاهده و بررسی تراکنش‌های افزایش اعتبار و شارژ کیف پول کاربران با مبالغ انتخابی
        </p>
      </div>
      ...
      <div className="border border-[var(--color-border)] rounded-2xl p-4 bg-[var(--color-surface)] shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <form onSubmit={handleSearchSubmit} className="relative w-full sm:w-80">
          <input
            type="text"
            placeholder="جستجو با ایمیل، نام، شماره پیگیری..."
            value={search}
      ...
      >
        {isLoading ? (
          ...
          <AdminEmptyState message="هیچ تراکنش شارژ کیف پولی مطابق با فیلترها یافت نشد." />
        ) : (
          payments.map((p) => {
            ...
            return (
              ...
              >
                ...
                {/* Top-up Amount */}
                <td className="px-4 py-3 whitespace-nowrap font-bold text-xs text-emerald-600 dark:text-emerald-400">
                  <div className="flex items-center gap-1">
                    <ArrowUpRight className="w-3.5 h-3.5 shrink-0" />
                    <span>{formatAmountOnly(p.amount)}</span>
                  </div>
                </td>

                {/* Order / Transaction ID */}
                <td className="px-4 py-3 whitespace-nowrap font-mono text-xs">
                  <span className="font-bold text-[var(--color-text)]" title={p.transactionId ? `تراکنش: ${p.transactionId}` : undefined}>
                ...
                <td className="px-4 py-3 text-xs whitespace-nowrap">
                  {p.sourceCardLast4 ? (
                    <span className="font-mono text-amber-600 dark:text-amber-300 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20 text-[11px]" title={p.payerName ? `واریزکننده: ${p.payerName}` : undefined}>
                      **** {p.sourceCardLast4}
                    </span>
                  ) : p.trackingNumber ? (
                    <span className="font-mono text-[var(--color-text)] text-[11px]" title={p.payerName ? `واریزکننده: ${p.payerName}` : undefined}>
                      {p.trackingNumber}
                    </span>
                  ) : (
                ...
                {/* Date */}
                <td className="px-4 py-3 whitespace-nowrap text-xs text-[var(--color-text-muted)]" title={`ثبت: ${formatPersianDate(p.createdAt)}${p.paidAt ? ` | شارژ: ${formatPersianDate(p.paidAt)}` : ""}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-[var(--color-text)]">
              بررسی و تأیید شارژ کارت‌به‌کارت کیف پول
            </h3>
            <div className="space-y-2 text-xs text-[var(--color-text)] bg-[var(--color-surface-warm)] p-4 rounded-xl border border-[var(--color-border)]">
              ...
              <div>
                <span className="text-[var(--color-text-muted)]">مبلغ شارژ: </span>
                <span className="font-bold text-emerald-600">{formatToman(inspectingPayment.amount)} تومان</span>
              </div>
              <div>
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-[var(--color-border)]">
              ...
              >
                ...
                <span>تأیید پرداخت و شارژ موجودی کیف پول</span>
              </button>
            </div>
          </div>
        </div>

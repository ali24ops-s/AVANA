...
export function AdminUserPurchasesPage() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "خطا در دریافت لیست خریدهای محصولات";
      setErrorMsg(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const getProductTypeIcon = (type?: string) => {
    if (type === "course") return <BookOpen className="w-3.5 h-3.5 text-indigo-500" />;
    return <Package className="w-3.5 h-3.5 text-teal-600" />;
  };
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {/* Header */}
      <PageHeader
        title="خرید محصولات کاربران"
        badge={{
          text: "مدیریت خریدهای کاربران",
          icon: <ShoppingBag className="w-3.5 h-3.5 shrink-0" />,
        }}
        description="مشاهده و مدیریت تراکنش‌های خرید دوره‌ها، بسته‌های آموزشی و سایر محصولات پلتفرم"
      />
      ...
      <div className="border border-[var(--color-border)] rounded-2xl p-4 bg-[var(--color-surface)] shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <form onSubmit={handleSearchSubmit} className="relative w-full sm:w-80">
          <input
            type="text"
            placeholder="جستجو با ایمیل، نام محصول، شماره سفارش..."
            value={search}
      ...
      >
        {isLoading ? (
          ...
          <AdminEmptyState message="هیچ تراکنش خرید محصولی مطابق با فیلترها یافت نشد." />
        ) : (
          payments.map((p) => {
              ...
              >
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex items-center gap-1.5 text-xs font-medium text-[var(--color-text)]">
                    {getProductTypeIcon(p.productType)}
                    <span className="truncate max-w-[220px]" title={p.productTitle || "محصول آموزشی"}>
                      {p.productTitle || "محصول آموزشی"}
                    </span>
                  </div>
                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-[var(--color-text)]">
              بررسی و تأیید پرداخت کارت‌به‌کارت خرید محصول
            </h3>
            <div className="space-y-2 text-xs text-[var(--color-text)] bg-[var(--color-surface-warm)] p-4 rounded-xl border border-[var(--color-border)]">
              ...
              <div>
                <span className="text-[var(--color-text-muted)]">محصول: </span>
                <span className="font-bold">{inspectingPayment.productTitle || "محصول آموزشی"}</span>
              </div>
              <div>
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-[var(--color-border)]">
              ...
              >
                ...
                <span>تأیید پرداخت و اعطای دسترسی محصول</span>
              </button>
            </div>
          </div>
        </div>

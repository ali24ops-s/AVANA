...
export const LANGUAGE_REQUIREMENT_PROMPT = [
  ...
  ``,
  `MEDICINAL CHEMISTRY & CHEMICAL STRUCTURE POLICY:`,
  `- Mandatory Chemical Structure Generation for Named Compounds:`,
  `  * Whenever a specific drug or chemical compound is mentioned by name in the educational lesson content (e.g., Histamine, Cimetidine, Ranitidine, Famotidine, Diphenhydramine, Dimenhydrinate, Chlorpheniramine, Cetirizine, Lidocaine, etc.), you MUST provide its chemical structure using a clean \`\`\`chemical block in Persian Markdown right next to where it is introduced or explained.`,
  `  * Structure generation is NOT limited to pure SAR sections; any specific named drug discussed in the lesson should include its 2D chemical structure block.`,
  `  * Example Format:`,
  `    \`\`\`chemical`,
  ...
  `    \`\`\``,
  `- Strict SMILES & Formula Precision (A syntactically valid SMILES is not sufficient):`,
  `  * A syntactically valid SMILES is NOT sufficient. It must also be strictly consistent with the stated compound identity and formula.`,
  `  * Verify heavy-atom count parity: The count of C, N, O, S, Cl, F atoms in your SMILES must EXACTLY match the numbers in the stated \`formula\`.`,
  `  * NEVER generate truncated or approximate SMILES (e.g. omitting a methylene -CH2- carbon or a substituent methyl).`,
  `  * Canonical References for Common Prototype Drugs:`,
  `    - Histamine (C5H9N3, 5 Carbons): smiles: NCCc1c[nH]cn1 (ethylamine chain + 3-carbon imidazole ring = 5 carbons total)`,
  `    - Cimetidine (C10H16N6S, 10 Carbons): smiles: Cc1c(CSCCN=C(NC)NC#N)nc[nH]1 or CNC(=NC#N)NCCSCc1c[nH]cn1C`,
  `    - Diphenhydramine (C17H21NO, 17 Carbons): smiles: CN(C)CCOC(c1ccccc1)c2ccccc2`,
  `    - Ranitidine (C13H22N4O3S, 13 Carbons): smiles: CNC(=C[N+](=O)[O-])NCCSCc1ccc(CN(C)C)o1`,
  `- Contextual Placement & Deduplication:`,
  `  * Place each structure block right next to where the drug/compound is introduced or analyzed in the text.`,
  `  * Do NOT repeat the chemical block every time the drug name is mentioned again in the same lesson; generate it once in its primary educational context.`,
  `- Strict Anti-Hallucination & Verifiability Guard:`,
  `  * A drug name in the lesson is a TRIGGER to attempt providing its verified structure, NOT a license to invent or speculate SMILES strings.`,
  `  * NEVER guess or hallucinate speculative SMILES. Only provide standard, valid, verifiable SMILES for well-known verified drugs.`,
  `  * If you are genuinely uncertain about the exact canonical SMILES syntax or heavy-atom count of a compound, omit the chemical block for that specific compound. An omitted block is acceptable; an incorrect or mismatched structure is NOT acceptable.`,
  `- Excluded Non-Compound Entities:`,
  `  * Do NOT generate chemical blocks for broad drug classes without a specific molecule (e.g. "آنتی‌هیستامین‌ها"), receptors (H1/H2), diseases, symptoms, biological pathways, proteins, or references.`,
].join("\n");
...
export function buildLessonGenerationUserPrompt(
  ...
  return [
    ...
    `⸻`,
    ``,
    `12.1. MEDICINAL CHEMISTRY & CHEMICAL STRUCTURE POLICY`,
    ``,
    `Whenever a specific drug or chemical compound is mentioned by name in the educational lesson content (e.g., Histamine, Cimetidine, Ranitidine, Famotidine, Diphenhydramine, Dimenhydrinate, Chlorpheniramine, Cetirizine, Lidocaine, etc.), you MUST provide its chemical structure using a clean \`\`\`chemical block in Persian Markdown right next to where it is introduced or explained.`,
    ``,
    `Structure generation is NOT limited to pure SAR sections; any specific named drug discussed in the lesson should include its 2D chemical structure block.`,

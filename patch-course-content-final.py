"""Idempotent patch for CourseContentPage.tsx — PR5-C3 lesson creation."""
...
if old_lesson_editor_import in content:
    ...
    applied.append("imports (NewLessonDialog, CreateContentLessonRequest)")
else:
    skipped.append("imports NewLessonDialog/CreateContentLessonRequest")
...
new_hooks = """  const orgQuery = useOrganization();
  ...
  const createLessonMutation = useMutation({
    mutationFn: (data: CreateContentLessonRequest & { moduleId: string }) => {
      const { moduleId, ...body } = data;
